...
- 📝 **添削**: 英語の間違いを最小限だけ直し、決まった日本語の説明を添えて表示(説明できない直しは出しません)
- 📚 **単語学習**: AIが拾った単語カードを「これ覚えたい」で保存(単語カードは標準モードのみで、出ないことが多いです)
...
- **メモリ 8GB 以上**(16GB 以上推奨)
  ...
  - **既定は自動で「軽量モード」**(後述)になります。返答は 1〜2 文(最初の挨拶だけは 3 文まで)で、
    **日本語訳と添削は出ますが、単語カードは出ません**
  - 大きいモデル(`llama3.2:3b` 以上)を自分で取得して選ぶと標準モードになりますが、
    評価では `llama3.2:3b` の日本語訳は同梱モデルより良くならず、単語カードもほとんど出ませんでした。
    Gemma 2 9B / Qwen 2.5 14B も選べますが、精度は計測していません
- **空き容量 6GB 以上推奨**(DMG 約 1.7GB + アプリ展開 + ランタイム同期分)
...
> DMG をインストールするだけで使えます(モデル DL 不要)。
> DMG のサイズは約 1.7GB です(v1.2.1 の実測。v1.1.0 までは 3B を同梱して約 2.7GB ありました)。
> GitHub Releases は 2GB 上限のため、DMG は Google ドライブで配布しています(ダウンロードリンクは GitHub のリリースページにあります)。

1. Speaky.dmg(約 1.7GB)をダウンロードして開き、Applications にドラッグ
...
4. メインウィンドウが開いたら、オンボーディング画面で **AI キャラクター(名前 / 性別)**
   ...
   - オンボーディングも設定画面も、追加のモデルのダウンロードは薦めません
     (同梱の 1.5B で日本語訳・添削つきの会話がそのまま始められます)

### Gatekeeper の警告が出た場合

配布版は Apple の Developer ID で署名・公証済みなので、通常は警告なしで開けます。
もし「開発元を確認できない」などの警告が出た場合は、以下の手順で許可してください:
...
6. 単語カードが出てきたら **「♡ これ覚えたい」** で復習リストに保存(添削は保存できません)
...
| 単語カード  | 出ることがある(評価では llama3.2:3b でもほとんど出ない) | **出さない**(小さいモデルの単語抽出は役に立たないことが多いため) |
| 添削        | 出す                                                    | **出す**(変わりません。下の「添削のしくみ」)                     |
...
単語カードが出ません(日本語訳と添削は出ます)。
LLM を選び直すと、会話モードの固定は「自動」に戻ります
(前のモデル向けの固定を別のモデルに引きずらないため)。

### 添削のしくみ

添削は、会話の返答とは別の短い呼び出しで作ります。AI には「文法を最小限だけ直す。
正しければそのまま繰り返す」ことだけを頼み、説明は書かせません。返ってきた文と話した文の
違いを単語単位で調べて、**すべての変更に決まった日本語の説明を付けられたときだけ**表示します
(言い換え・固有名詞や数字の変更・説明を用意していない種類の直しは表示しません)。

- 返答の読み上げ・マイクは添削を待ちません(日本語訳の後に届きます)
- 3 語未満 / 25 語を超える発話、日本語や英日混在の発話は添削しません
- 140 件のラベル付き発話での評価(`qwen2.5:1.5b` / `llama3.2:3b` とも同じ結果):
  正しい文を書き換えて表示したのは **78 件中 0 件**、間違いを正しく直して表示したのは **62 件中 49 件**、
  間違った説明は **0 件**。0/78 は「3.8% 以下」までしか言えない数字です(95% 信頼区間の上限)

### 選べる LLM

| モデル         | サイズ | 向き                                                                              |
| -------------- | ------ | --------------------------------------------------------------------------------- |
| `qwen2.5:1.5b` | ~1GB   | **同梱・既定**。8GB 機向け。日本語訳が安定(軽量モード = 添削あり・単語カードなし) |
| `gemma2:2b`    | ~1.6GB | 要 DL。会話は成立する(軽量モード。添削・日本語訳の品質は未計測)                   |
| `llama3.2:3b`  | ~2GB   | 要 DL。標準モード。日本語訳は同梱モデルより良くならない(評価済み)                 |
| `gemma2:9b`    | ~5.5GB | 要 DL。16GB 以上向け。重い(精度は未計測)                                          |
| `qwen2.5:14b`  | ~9GB   | 要 DL。16GB 以上向け。低速(精度は未計測)                                          |
...
ollama pull llama3.2:3b   # 標準モードで使いたいとき
```

**v1.1.0 以前から使っている場合**: 既に取得済みの `llama3.2:3b` はアップグレードしても
消えません(同梱物の同期は追加のみで、既存のモデルを削除しません)。
ただし `llama3.2:3b`(v1.1.0 の既定)を選んだままだった場合は、アップグレード後の最初の起動で
**一度だけ**同梱の `qwen2.5:1.5b` に切り替わり、画面左下にお知らせが出ます(日本語訳が安定するため)。
3B に戻したいときは設定画面の「LLM」で選び直してください。以後は切り替わりません。
同梱モデルが入っていない構成(自前の Ollama を使っている等)では切り替わりません。
...
# テスト(frontend 359件 / backend 819件)
npm test
