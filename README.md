...
- **メモリ 8GB 以上**(16GB 以上推奨)
  ...
  - 大きいモデル(`llama3.2:3b` 以上)を自分で取得して選ぶと標準モードになり単語カードが出ますが、
    評価では `llama3.2:3b` の日本語訳は同梱モデルより良くなりませんでした。
...
| 単語カード  | 出す                   | **出さない**(小さいモデルの単語抽出は役に立たないことが多いため) |
...
| `llama3.2:3b`  | ~2GB   | 要 DL。標準モード(単語カードあり)。日本語訳は同梱モデルより良くならない(評価済み) |
...
ollama pull llama3.2:3b   # 標準モード(単語カードあり)で使いたいとき
```
