 ...
 *
 * v1.2.0 以降は同梱物を軽量モデル(現在は qwen2.5:1.5b)に変えたが、それだけでは同じ形の穴がまた開く
 ...
 *
 * - メモリが少ない / 分からない: 軽いものから。同梱の軽量モデルが先頭。
 * - メモリに余裕がある: 3B を先頭にして、無ければ同梱の軽量モデルに落ちる。
 *   取得の選択肢から外したモデル(llama3.2:1b 等)はここに並ばないので、
 *   v1.2.0 のテスト機のように旧同梱物と新同梱物が両方あれば新同梱物が選ばれる。
